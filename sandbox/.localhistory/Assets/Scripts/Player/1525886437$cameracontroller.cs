...
public class CameraController : MonoBehaviour {
	...
	void Update ()
    {
        ...
        if (Physics.Raycast(transform.position, -(transform.position - Target.position).normalized, out hit, Zoom))
        {
            Target.localPosition = new Vector3(Target.localPosition.x, Target.localPosition.y, -hit.distance + 2);
            print(hit.distance);
        }
        else
        {
            Target.localPosition = new Vector3(Target.localPosition.x, Target.localPosition.y, Mathf.Lerp(Target.localPosition.z, -Zoom,.1f));
