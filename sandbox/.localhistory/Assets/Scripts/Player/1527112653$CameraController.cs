    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, -(transform.position - CamTarget.position).normalized, out hit, Zoom))
        {
            if(hit.transform.gameObject.tag != "Player")
             CamTarget.localPosition = new Vector3(CamTarget.localPosition.x, transform.localPosition.y, -hit.distance);
        }
        else
        {
            CamTarget.localPosition = new Vector3(CamTarget.localPosition.x, transform.localPosition.y, Mathf.Lerp(CamTarget.localPosition.z, -Zoom, .1f));
        ...
        else
        {
            ...
            {
                tpos = new Vector3(tpos.x, Mathf.Lerp(tpos.y, PlayerTarget.position.y + Vector3.up.y, .1f), tpos.z);
            }
            else
            {
