...
public class CustomJoint : MonoBehaviour {
	...
	void Update ()
    {
        ...
        if (playAnim)
        {
            ...
            rotation += new Vector3(XMotionCurve.Evaluate(currentTime + animOffset) + (rotationLock.x * transform.eulerAngles.x), YMotionCurve.Evaluate(currentTime + animOffset), ZMotionCurve.Evaluate(currentTime + animOffset));
