...
public class CustomIKSolver : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < Joints.Length; i++)
        {
            ...
            {
                angles[i] = Joints[i].transform.rotation.eulerAngles.x;
            }
            else if (Joints[i].Axis.y != 0.0)
            {
                angles[i] = Joints[i].transform.rotation.eulerAngles.y;
            }
            else if (Joints[i].Axis.z != 0.0)
            {
                angles[i] = Joints[i].transform.rotation.eulerAngles.z;
            }
            angles[i] += Joints[i].AngleOffset;
        }
        InverseKinematics(target.position, angles);

        for (int i = 0; i < Joints.Length; i++)
        {
                Joints[i].transform.rotation = Quaternion.Slerp (Joints[i].transform.rotation,Quaternion.AngleAxis(angles[i],Joints[i].Axis),.8f);
        }
    }
