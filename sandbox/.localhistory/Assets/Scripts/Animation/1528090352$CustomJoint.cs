...
public class CustomJoint : MonoBehaviour {
    ...
    void OnDrawGizmos()
    {
        ...
        UltiDraw.Begin();
        UltiDraw.DrawBone(transform.position, Quaternion.LookRotation(-(transform.localPosition).normalized,transform.localToWorldMatrix(Vector3.up)),0.5f, (transform.position - transform.parent.position).magnitude, UltiDraw.IndianRed);
        UltiDraw.End();
    }
