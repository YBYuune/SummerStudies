...
public class CustomJoint : MonoBehaviour {
    ...
    void OnRenderObject()
    {
        ...
        UltiDraw.Begin();
        UltiDraw.DrawBone(transform.position, Quaternion.LookRotation((transform.position - transform.parent.position).normalized),0.5f, (transform.position - transform.parent.position).magnitude, UltiDraw.Purple.Transparent(0.5f));
        UltiDraw.End();
    }
