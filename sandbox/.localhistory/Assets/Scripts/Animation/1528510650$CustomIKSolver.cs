    ...
    private void Update()
    {
        ...
        for (int i = 0; i < Joints.Length; i++)
        {
                Joints[i].transform.rotation = Quaternion.Slerp (Joints[i].transform.rotation,
                    Quaternion.FromToRotation(GetPivot() - Joints[i].transform.position,target.position - Joints[i].transform.position) * Joints[i].transform.rotation * Quaternion.AngleAxis(Joints[i].AngleOffset,Joints[i].Axis),
