...
public class CustomJoint : MonoBehaviour {
    ...
    void OnDrawGizmos()
    {
        ...
        UltiDraw.Begin();
        UltiDraw.DrawBone(transform.position, Quaternion.LookRotation(-(transform.localPosition).normalized,transform.parent.up),0.5f, (transform.position - transform.parent.position).magnitude, UltiDraw.Purple.Transparent(0.5f));
        UltiDraw.End();
    }
