...
public class CustomIKSolver : MonoBehaviour {
    ...
    public Vector3 TargetMultiplier = Vector3.one;
        ...
        for (int k = 0; k < Itterations; k++)
        {
            for (int i = 0; i < Joints.Length; i++)
            {
                Joints[i].transform.rotation = Quaternion.Slerp(Joints[i].transform.rotation,
                    Quaternion.FromToRotation(GetPivot() - Joints[i].transform.position, ((target.position + TargetOffset)* TargetMultiplier) - Joints[i].transform.position) * Joints[i].transform.rotation,
