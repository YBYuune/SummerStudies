...
public class CustomIKSolver : MonoBehaviour {
    ...
    public Vector3 TargetOffset = new Vector3(0, -9.574014f, 0);
        ...
        for (int k = 0; k < 3; k++)
        {
            for (int i = 0; i < Joints.Length; i++)
            {
                Joints[i].transform.rotation = Quaternion.Slerp(Joints[i].transform.rotation,
                    Quaternion.FromToRotation(GetPivot() - Joints[i].transform.position, target.position - Joints[i].transform.position) * Joints[i].transform.rotation,
