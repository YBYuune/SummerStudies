...
public class CustomJoint : MonoBehaviour {
    ...
    {
        UltiDraw.Begin();
        UltiDraw.DrawBone(transform.position, Quaternion.LookRotation((transform.position - parentJoint.position).normalized),0.5f, (transform.position - parentJoint.position).magnitude, UltiDraw.Purple.Transparent(0.5f));
        UltiDraw.End();
    }
