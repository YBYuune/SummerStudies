Shader "Casey-Screen/CannyEdgeDetect-NonMaxSupression"
{
	Properties
	{
		...
		_LowerThreshold("Lower Threshold", Float) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 sobel = textures[4];//tex2D(_MainTex, i.uv);
				...
				return fixed4(sdSphere(FragColor.rgb, 1), sdSphere(FragColor.rgb, 1), sdSphere(FragColor.rgb, 1),1);
			}
			ENDCG
		}
	}
