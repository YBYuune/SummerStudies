...
#include "UnityCG.cginc"
	...
	{
		result = {	tex[index - 6],	tex[index - 5],	tex[index - 4],
	...
	fixed3 ApplySobel(fixed3 textures[9]) // applys the sobel xy kernel's on a grayscale version of the image passed in
	{
		...
		for (int i = 0, k = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++, k++)
			{
				float Gx = KERNEL_SOBELX[i][j] * (textures[k]);
				float Gy = KERNEL_SOBELY[i][j] * (textures[k]);
	...
	for (int j = 0; j < 9; j++)
	{
		...
		textures[j] = Grayscale(tex2D(_MainTex, i.uv + offsets[j]).rgb);
	}
	...
	for (j = 0; j < 25; j++)
	{
		...
		textures5x5[j] = Grayscale(tex2D(_MainTex, i.uv + offsets5x5[j]).rgb);
	}
	...
	else
	{
		...
		void Get3x3From5x5(textures5x5, 6, t00);
		void Get3x3From5x5(textures5x5, 7, t01);
		void Get3x3From5x5(textures5x5, 8, t02);

		void Get3x3From5x5(textures5x5, 6 + 6, t10);
		void Get3x3From5x5(textures5x5, 7 + 6, t11);
		void Get3x3From5x5(textures5x5, 8 + 6, t12);

		void Get3x3From5x5(textures5x5, 6 + 12, t20);
		void Get3x3From5x5(textures5x5, 7 + 12, t21);
		void Get3x3From5x5(textures5x5, 8 + 12, t22);


		FragColor = fixed4((ApplyKernel3x3(KERNEL_GBLUR,textures)), 1.0);
	}
	return FragColor;
	}
		ENDCG
