...
inline void GetTextues3x3(float2[9] offset, out fixed3 textures[9], float2 uv)
{
	fixed3 t[9];
	for (int j = 0; j < 9; j++)
	{
		t[j] = tex2D(_MainTex, uv + offset[j]).rgb;
	}
	textures = t;
}

inline void GetTextues3x3(float2[9] offset, out fixed3 textures[9], float2 uv, sampler2D tex)
{
	fixed3 t[9];
	for (int j = 0; j < 9; j++)
	{
		t[j] = tex2D(tex, uv + offset[j]).rgb;
	}
	textures = t;
}

inline void GetTextues5x5(float2[25] offset, out fixed3 textures[25], float2 uv)
{
	fixed3 t[25];
	for (j = 0; j < 25; j++)
	{
...
inline void GetTextues5x5(float2[25] offset, out fixed3 textures[25], float2 uv, sampler2D tex)
{
	fixed3 t[25];
	for (j = 0; j < 25; j++)
	{
