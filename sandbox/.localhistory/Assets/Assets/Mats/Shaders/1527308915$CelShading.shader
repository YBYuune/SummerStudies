Shader "Custom/CelShading" 
{
	...
	Properties{
		...
		_Specular("Specular Intensity", Range(0, 128)) = 0
	...
	SubShader{
		...
		half4 LightingCelShading(SurfaceOutput s, half3 lightDir, half atten) {
			...
			float nh = dot(s.Normal, dot(s.Normal, lightDir));
			...
			half4 c;
			c.rgb = (s.Albedo * _LightColor0.rgb + _LightColor0.rgb * spec)  * (NdotL * TAtten);
			c.a = s.Alpha;
			return c;
		}
